...
protocol FaseServiceProtocol: GenericService {
    func getAllListaDeCampeonatos(id: Int, completion: @escaping completion<[FasesCopaElement]?>)
}

class FaseService: FaseServiceProtocol {
    ...
    func getAllListaDeCampeonatos(id: Int ,completion: @escaping completion<[FasesCopaElement]?>) {
        guard let url = URL(string: "\(baseUrl)campeonatos/\(id)/fases") else {
        ...
        let task = URLSession.shared.dataTask(with: request) { data, response, error in
            ...
            do {
                let decodeData = try JSONDecoder().decode([FasesCopaElement].self, from: data)
